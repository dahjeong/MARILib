...
def cabin_pressure_grad(altp_d):
    ...
    pamb, pamb_d, tamb, tamb_d, dtodz = g_earth.atmosphere_grad(
        altp, altp_d, disa, disa_d)
...
def air(xg, zg, xg_d, zg_d):
    ...
    pamb, pamb_d, tamb, tamb_d, dtodz, dtodz_d = g_earth.atmosphere_geo_grad(
        zg, zg_d, disa, disa_d)  # Standard atmosphere
...
def state_dot(xin, state, rating, nei, aircraft):
    ...
    rho, rho_d, sig, sig_d = g_earth.air_density_grad(
        pamb, pamb_d, tamb, tamb_d)
...
#-------------------------------------------------------------------------
    ...
    cx, lod = airplane_aero.drag(
        aircraft, pamb, tamb, mach, cz)    # Aerodynamic drag model

    path_d = ((0.5 * rho * vair**2) * area * cz - mass * g *
              numpy.cos(path)) / (mass * vair)    # Lift equation
    vgnd_d = (fn - mass * g * numpy.sin(path) - (0.5 * rho * vair**2)
              * area * cx) / mass      # Drag equation
...
def flight_point(aircraft, rating, nei, state, var, input, dat, cmd, dof, cst):
    ...
    #=========================================================================
    def fct_flight_point(
            var, state, input, dat, cmd, dof, cst, nei, rating, aircraft):
...
#=========================================================================
def flight_segment(
        aircraft, rating, nei, state, stop, input, dat, cmd, dof, cst, dt, flight):
    ...
    while (eval("xout[stop_idx]" + op + "stop_val")):
        ...
        xout, state, state_d = flight_point(
            aircraft, rating, nei, state, var, input, dat, cmd, dof, cst)
...
#=========================================================================
def iso_cas_climb(
        aircraft, vcas1, altp1, vcas2, altp2, mach, vz_mcl_min, flight):
    """
    Constant CAS climb :
    ...
    if(vcas1 > vcas2):
        raise Exception(
            "iso_cas_climb, vcas2 must be higher or equal than vcas1")

    if(altp0 > altp1):
        raise Exception(
            "iso_cas_climb, altp1 must be higher than starting altitude")
    ...
    if(altp1 > altp_cross_over):
        raise Exception(
            "iso_cas_climb, cross over altitude must be higher than altp1")
    ...
    flight, state = flight_segment(
        aircraft, MCL, nei, state, stop, input, dat, cmd, dof, cst, dt, flight)
    ...
    if(flight[-1, xout_dict["vzair"]] < vz_mcl_min):
        raise Exception(
            "iso_cas_climb, final altitude cannot be reached with minimum required vertical speed")
...
#=========================================================================
def level_flight(
        aircraft, mach, vz_mcl_min, vz_mcr_min, heading, stop, flight):

    westbound = numpy.concatenate(
        (numpy.arange(
            2000., 41000., 2000.), numpy.arange(
            43000., 85000., 4000.)))
    eastbound = numpy.concatenate(
        (numpy.arange(
            1000., 42000., 2000.), numpy.arange(
            45000., 87000., 4000.)))
    ...
    #-------------------------------------------------------------------------
    sar, sar_up = test_air_range(
        aircraft, MCR, nei, mach, altp, altp_up, state)
    ...
    while (eval("flight[-1,stop_idx]" + op + "stop_val")):
        ...
        if (sar < sar_up):      # Efficiency is better on upper level
            ...
            # Ceilings allow to climb
            if (vz_mcr_min < vz_mcr and vz_mcl_min < vz_mcl):

                if (0 < active_ceiling):
                    # Compute transition point according to climb speed
                    #----------------------------------------------------------
                    if (active_ceiling == 1):
                        time = flight[-1, xout_dict["time"]] - dtime / \
                            (1 + numpy.abs((vz_mcr_min - active_vz) /
                                           (vz_mcr - vz_mcr_min)))
                    else:
                        time = flight[-1, xout_dict["time"]] - dtime / \
                            (1 + numpy.abs((vz_mcl_min - active_vz) /
                                           (vz_mcl - vz_mcl_min)))
                else:
                    # Compute transition point according to SAR
                    #----------------------------------------------------------
                    time = flight[-1, xout_dict["time"]] - dtime / \
                        (1 + numpy.abs((sar0 - sar_up0) / (sar_up - sar)))
                ...
                dat = numpy.array(
                    [["fn", "fct_thrust(aircraft,state,rating,nei)"]])
                cmd = numpy.array(["cz"])
                dof = numpy.array(["vgnd", "path"])
                cst = numpy.array(
                    [["mach", "input[0]"], ["mach_d", "0"], ["path_d", "0"]])
                stop = numpy.array(["altp", "<", "input[1]"])
                ...
                cst = numpy.array([["altp", "input[0]"], ["altp_d", "0"], [
                                  "mach", "input[1]"], ["mach_d", "0"], ["path_d", "0"]])
                stop = numpy.array(["time", "<", "input[2]"])

                flight, state = flight_segment(
                    aircraft, MCR, nei, state, stop, input, dat, cmd, dof, cst, dt, flight)
...
def standard_descent(aircraft, altp0, vcas1, altp1, vcas2, mach, flight):
    ...
    flight, state = flight_segment(
        aircraft, FID, nei, state, stop, input, dat, cmd, dof, cst, dt, flight)
...
def qs_mission(aircraft, disa, tow, range, heading,
    ...
    flight, state = iso_cas_climb(
        aircraft, vcas1_clb, altp1_clb, vcas2_clb, altp2_clb, cruise_mach, vz_mcl_min, flight)

    stop = numpy.array(["xg", "<", str(range)])

    flight, state = level_flight(
        aircraft, cruise_mach, vz_mcl_min, vz_mcr_min, heading, stop, flight)

    flight, state = standard_descent(
        aircraft, altp0_dsc, vcas1_dsc, altp1_dsc, vcas2_dsc, cruise_mach, flight)
...
def test_ceilings(aircraft, nei, mach, altp, state):
    ...
    dof = numpy.array(["zg", "vgnd", "path"])
    cst = numpy.array(
        [["altp", "input[0]"], ["mach", "input[1]"], ["mach_d", "0"], ["path_d", "0"]])

    var = init_var(aircraft, state, cmd, dof)

    xout, state, state_d = flight_point(
        aircraft, MCR, nei, state_copy, var, input, dat, cmd, dof, cst)
    vz_mcr = xout[xout_dict["vzair"]]

    var = init_var(aircraft, state, cmd, dof)

    xout, state, state_d = flight_point(
        aircraft, MCL, nei, state_copy, var, input, dat, cmd, dof, cst)
...
def test_air_range(aircraft, rating, nei, mach, altp, altp_up, state):
    ...
    input = numpy.array([altp, mach])
    xout, state, state_d = flight_point(
        aircraft, rating, nei, state_copy, var, input, dat, cmd, dof, cst)
    sar = - state_d[state_dict["xg"]] / state_d[state_dict["mass"]]

    var = init_var(aircraft, state, cmd, dof)

    input = numpy.array([altp_up, mach])
    xout_up, state_up, state_d_up = flight_point(
        aircraft, rating, nei, state_copy, var, input, dat, cmd, dof, cst)
