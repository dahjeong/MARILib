...
def one_airframe_price(aircraft):
    ...
    airframe_price = 0.7e3 * \
        (9e4 + 1.15 * weights.mwe - 1.8e9 / (2e4 + weights.mwe**0.94))
...
def eval_operating_costs(aircraft, block_fuel, block_time):
    ...
    eco.elec_cost = weights.battery * \
        propulsion.battery_energy_density * eco.elec_price

    b_h = block_time / 3600.
    t_t = b_h + 0.25
    w_f = (10000. + weights.mwe - propulsion.mass) * 1e-5

    labor_frame = ((1.26 + 1.774 * w_f - 0.1071 * w_f**2) *
                   t_t + (1.614 + 0.7227 * w_f + 0.1204 * w_f**2)) * labor_cost
    matrl_frame = (12.39 + 29.8 * w_f + 0.1806 * w_f**2) * \
        t_t + (15.20 + 97.330 * w_f - 2.8620 * w_f**2)
    frame_cost = labor_frame + matrl_frame

    t_h = 0.05 * (propulsion.reference_thrust_effective / 4.4482198) * 1e-4

    labor_engine = engine.n_engine * \
        (0.645 * t_t + t_h * (0.566 * t_t + 0.434)) * labor_cost
    ...
    eco.navigation_fees = 57. * \
        (cost_mission.range / 185200.) * \
        numpy.sqrt((weights.mtow / 1000.) / 50.)
    ...
    std_op_cost = eco.fuel_cost + eco.elec_cost + frame_cost + engine_cost + \
        eco.cockpit_crew_cost + eco.landing_fees + eco.navigation_fees

    cash_op_cost = std_op_cost + eco.cabin_crew_cost + \
        eco.catering_cost + eco.pax_handling_cost + eco.ramp_handling_cost
    ...
    aircraft_price = frame_price + engine_price * \
        engine.n_engine + gear_price + battery_price

    eco.total_investment = frame_price * 1.06 + \
        engine.n_engine * engine_price * 1.025

    eco.interest = (eco.total_investment / (utilisation * period)) * (irp *
                                                                      0.04 * (((1. + interest_rate)**irp) / ((1. + interest_rate)**irp - 1.)) - 1.)

    eco.insurance = 0.0035 * aircraft_price / utilisation

    eco.depreciation = 0.99 * \
        (eco.total_investment / (utilisation * period))     # Depreciation

    direct_op_cost = cash_op_cost + eco.interest + \
        eco.depreciation + eco.insurance
