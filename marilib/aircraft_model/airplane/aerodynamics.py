...
#           : hld_conf = 0.1 to 0.5 ==> cz_max_to
    ...
    # Assumed the Lift vs AoA is just translated upward and Cz0 clean equal to
    # zero
    cz_0 = cz_max - cz_max_base
...
def drag(aircraft, pamb, tamb, mach, cz):
    ...
    fuse_cxf = 1.05 * ((0.455 / fac) * (numpy.log(10) / numpy.log(re *
                                                                  fuselage.length))**2.58) * fuselage.net_wetted_area / wing.area
    wing_cxf = 1.4 * ((0.455 / fac) * (numpy.log(10) /
                                       numpy.log(re * wing.mac))**2.58) * wing.net_wetted_area / wing.area
    htp_cxf = 1.4 * ((0.455 / fac) * (numpy.log(10) /
                                      numpy.log(re * htp.mac))**2.58) * htp.net_wetted_area / wing.area
    vtp_cxf = 1.4 * ((0.455 / fac) * (numpy.log(10) /
                                      numpy.log(re * vtp.mac))**2.58) * vtp.net_wetted_area / wing.area
    ...
    # Parasitic drag factor
    aircraft_kp = (0.0247 * aircraft_knwa - 0.11) * aircraft_knwa + 0.166
    ...
    cx_tap_base = lin_interp_1d(
        param,
        X,
        Y)     # Tapered fuselage drag (tail cone)

    # Effect of tail cone fan
    fuse_cx_tapered = cx_tap_base * propu.tail_cone_drag_effect(aircraft)
    ...
    # Induced drag
    #-------------------------------------------------------------------------
    ki = ((fuselage.width / wing.span)**2 + 1.05) / \
        (numpy.pi * wing.aspect_ratio)
    ...
    cz_design = 0.5
    aircraft_mach_div = 0.03 + \
        design_driver.cruise_mach + 0.1 * (cz_design - cz)
