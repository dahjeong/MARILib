...
def atmosphere_grad(altp, altp_d, disa, disa_d):
    ...
    def fct_atm_grad_t_p(j, T, T_d, P, P_d):
        ...
        T_d[j + 1] = T_d[j] + dtodz_d[j] * \
            (Z[j + 1] - Z[j]) + dtodz[j] * (Z_d[j + 1] - Z_d[j])
        if (0. < numpy.abs(dtodz[j])):
            ...
            P[1 + j] = P[j] * B1**B2
            P_d[1 + j] = P[j] * (B2 * B1_dz * B1**(B2 - 1.) + (B1**B2)
                                 * (B2_dt * numpy.log(B1) + B2 * B1_dt / B1)) + P_d[j] * B1**B2
        else:
            B3 = -(g / R) * ((Z[1 + j] - Z[j]) / (T[j] + disa))
            B3_d = -(g / R) * ((Z_d[1 + j] - Z_d[j]) / (T[j] + disa)) + (
                g / R) * ((Z[1 + j] - Z[j]) * (T_d[j] + disa_d) / (T[j] + disa)**2)
    ...
    def fct_atm_grad_amb(j, altp, altp_d, disa, disa_d):
        if (0. < numpy.abs(dtodz[j])):
            ...
            B1_dt = (dtodz_d[j] * (altp - Z[j])) / (T[j] + disa) - \
                (dtodz[j] * (altp - Z[j]) * disa_d) / (T[j] + disa)**2
            B2 = -g / (R * dtodz[j])
            B2_dt = (g * dtodz_d[j]) / (R * dtodz[j]**2)
            pamb = P[j] * B1**B2
            pamb_d = P[j] * (B2 * B1_dz * B1**(B2 - 1.) + (B1**B2) *
                             (B2_dt * numpy.log(B1) + B2 * B1_dt / B1)) + P_d[j] * B1**B2
        else:
            B3 = -(g / R) * ((altp - Z[j]) / (T[j] + disa))
            B3_d = -(g / R) * ((altp_d - Z_d[j]) / (T[j] + disa)) + (
                g / R) * ((altp - Z[j]) * (T_d[j] + disa_d) / (T[j] + disa)**2)
            pamb = P[j] * numpy.exp(B3)
            pamb_d = P_d[j] * numpy.exp(B3) + P[j] * B3_d * numpy.exp(B3)
        tamb = T[j] + dtodz[j] * (altp - Z[j]) + disa
        tamb_d = T_d[j] + dtodz_d[j] * \
            (altp - Z[j]) + dtodz[j] * (altp_d - Z_d[j]) + disa_d
    ...
    pamb, pamb_d, tamb, tamb_d = fct_atm_grad_amb(
        j, altp, altp_d, disa, disa_d)
...
def atmosphere_geo_grad(altg, altg_d, disa, disa_d):
    ...
    def fct_atm_geo_grad_amb(j, altg, altg_d, disa, disa_d):
        if (0. < numpy.abs(dtodz[j])):
            ...
            B1_dt = (dtodz_d[j] * (altg - Z[j])) / (T[j] + disa) - \
                (dtodz[j] * (altg - Z[j]) * disa_d) / (T[j] + disa)**2
            B2 = -g / (R * dtodz[j])
            B2_dt = (g * dtodz_d[j]) / (R * dtodz[j]**2)
            pamb = P[j] * B1**B2
            pamb_d = P[j] * (B2 * B1_dz * B1**(B2 - 1.) + (B1**B2) *
            ...
            B3_d = -(g / R) * ((altg_d - Z_d[j]) / (T[j] + disa)) + (
                g / R) * ((altg - Z[j]) * (T_d[j] + disa_d) / (T[j] + disa)**2)
            pamb = P[j] * numpy.exp(B3)
            pamb_d = P_d[j] * numpy.exp(B3) + P[j] * B3_d * numpy.exp(B3)
        tamb = T[j] + dtodz[j] * (altg - Z[j]) + disa
        tamb_d = T_d[j] + dtodz_d[j] * \
            (altg - Z[j]) + dtodz[j] * (altg_d - Z_d[j]) + disa_d
        return pamb, pamb_d, tamb, tamb_d
    #-------------------------------------------------------------------------
    ...
    while (j < n and Z[1 + j] <= altg):
        Z, Z_d, T, T_d, P, P_d = fct_atm_geo_grad_t_p(
            j, Z, Z_d, T, T_d, P, P_d)
        j = j + 1
    ...
    pamb, pamb_d, tamb, tamb_d = fct_atm_geo_grad_amb(
        j, altg, altg_d, disa, disa_d)
...
def pressure_altitude_grad(pamb, pamb_d):
    ...
    def fct_altp_grad_amb(j, pamb, T, P):
        if (0. < numpy.abs(dtodz[j])):
            altp = Z[
                j] + ((pamb / P[j])**(-(R * dtodz[j]) / g) - 1.) * (T[j] / dtodz[j])
            altp_d = (T[j] / dtodz[j]) * (-(R * dtodz[j]) / (g * P[j])
                                          ) * pamb_d * (pamb / P[j])**(-(R * dtodz[j]) / g - 1.)
        else:
    ...
    n = len(P) - 1
    P[1] = P[0] * \
        (1 + (dtodz[0] / T[0]) * (Z[1] - Z[0]))**(-g / (R * dtodz[0]))
