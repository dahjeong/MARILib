...
def atmosphere(altp, disa):
    ...
    if (0. < numpy.abs(dtodz[j])):
        pamb = P[j] * \
            (1 + (dtodz[j] / T[j]) * (altp - Z[j]))**(-g / (R * dtodz[j]))
    else:
...
def atmosphere_geo(altg, disa):
    ...
    while (j < n and Z[1 + j] <= altg):
        T[j + 1] = T[j] + dtodz[j] * (Z[j + 1] - Z[j])
        if (0. < numpy.abs(dtodz[j])):
            P[j + 1] = P[j] * (1. + (dtodz[j] / (T[j] + disa))
                               * (Z[j + 1] - Z[j]))**(-g / (R * dtodz[j]))
        else:
            P[j + 1] = P[j] * \
                numpy.exp(-(g / R) * ((Z[j + 1] - Z[j]) / (T[j] + disa)))
        j = j + 1
        K = 1 + disa / T[j]
        dtodz[j] = dtodzi[j] / K
    ...
    if (0. < numpy.abs(dtodz[j])):
        pamb = P[j] * (1 + (dtodz[j] / (T[j] + disa))
                       * (altg - Z[j]))**(-g / (R * dtodz[j]))
    else:
...
def mach_from_vcas(pamb, Vcas):
    ...
    fac = gam / (gam - 1.)
    mach = numpy.sqrt(((((((gam - 1.) / 2.) * (Vcas / vc0)**2 + 1) **
                         fac - 1.) * P0 / pamb + 1.)**(1. / fac) - 1.) * (2. / (gam - 1.)))
    return mach
...
def vcas_from_mach(pamb, mach):
    ...
    vcas = vc0 * \
        numpy.sqrt(
            5. * ((((pamb / P0) * ((1. + ((gam - 1.) / 2.) * mach**2)**fac - 1.)) + 1.)**(1. / fac) - 1.))
    return vcas
...
def cross_over_altp(Vcas, mach):
    ...
    pamb = ((1. + ((gam - 1.) / 2.) * (Vcas / vc0)**2)**fac - 1.) * \
        P0 / ((1. + ((gam - 1.) / 2.) * mach**2)**fac - 1.)
...
def climb_mode(speed_mode, dtodz, tstd, disa, mach):
    ...
    if (speed_mode == 1):
        ...
        acc_factor = 1. + (((1. + fac * mach**2)**(gam / (gam - 1.)) - 1.) / (1. + fac * mach**2)**(1. / (gam - 1.))) \
                        + ((gam * R) / (2. * g)) * (mach**2) * \
            (tstd / (tstd + disa)) * dtodz
    elif (speed_mode == 2):
        acc_factor = 1. + ((gam * R) / (2. * g)) * \
            (mach**2) * (tstd / (tstd + disa)) * dtodz
    else:
