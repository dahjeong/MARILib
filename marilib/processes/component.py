...
def eval_nominal_mission(aircraft):
    ...
    block_fuel, block_time, total_fuel = perfo.mission(
        aircraft, range, tow, altp, mach, disa)
...
def eval_mission_coupling(aircraft):
    ...
    aircraft.weights.mtow = aircraft.weights.owe + \
        aircraft.nominal_mission.payload + aircraft.nominal_mission.total_fuel
...
def mission_fuel_limited(aircraft, tow, total_fuel, altp, mach, disa):
    ...
    def fct_mission(range, aircraft, tow, total_fuel, altp, mach, disa):
        #======================================================================
        weights = aircraft.weights
        [block_fuel, block_time, fuel] = perfo.mission(
            aircraft, range, tow, altp, mach, disa)
        Y = total_fuel - fuel
        return Y
    #-------------------------------------------------------------------------
...
def eval_cost_mission(aircraft):
    ...
    tow, block_fuel, block_time, total_fuel = mission_tow(
    ...
    aircraft.cost_mission.block_CO2 = block_fuel * \
        aircraft.environmental_impact.CO2_index
...
def eval_economics(aircraft):
    ...
    direct_op_cost, cash_op_cost, battery_price, gear_price, engine_price, airplane_price = costing.eval_operating_costs(
...
def eval_take_off_performances(aircraft):
    ...
    aircraft.low_speed.perfo_constraint_1 = (
        aircraft.low_speed.req_tofl - tofl) / aircraft.low_speed.req_tofl
...
def eval_landing_performances(aircraft):
    ...
    aircraft.low_speed.perfo_constraint_2 = (
        aircraft.low_speed.req_app_speed - app_speed) / aircraft.low_speed.req_app_speed
