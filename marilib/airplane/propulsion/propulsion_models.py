...
def nacelle_drag(aircraft, Re, Mach):
    ...
    if (propulsion.architecture == "TF"):
        ...
        nacelle_cxf, nacelle_nwa = turbofan_nacelle_drag(
            aircraft, nacelle, Re, Mach)
    elif (propulsion.architecture == "PTE1"):
        nacelle = aircraft.turbofan_nacelle
        t_nacelle_cxf, t_nacelle_nwa = turbofan_nacelle_drag(
            aircraft, nacelle, Re, Mach)

        nacelle = aircraft.rear_electric_nacelle
        e_nacelle_cxf, e_nacelle_nwa = rear_electric_nacelle_drag(
            aircraft, nacelle, Re, Mach)
        ...
        nacelle_cxf, nacelle_nwa = electrofan_nacelle_drag(
            aircraft, nacelle, Re, Mach)
    else:
        raise Exception("propulsion.architecture index is out of range")
...
def thrust_pitch_moment(aircraft, fn, pamb, mach, dcx_oei):
    ...
    cm_prop = nacelle.z_ext * \
        (dcx_oei - fn / (0.5 * gam * pamb * mach**2 * wing.area))
...
def thrust_yaw_moment(aircraft, fn, pamb, mach, dcx_oei):
    ...
    cn_prop = (nacelle.y_ext / wing.mac) * \
        (fn / (0.5 * gam * pamb * mach**2 * wing.area) + dcx_oei)
