...
def pte1_sfc(aircraft, pamb, tamb, mach, rating, nei):
    ...
    eff_h = kC + (1. - kC) * \
        (kW * kBLIe * (eff_e_prop / eff_prop) * eff_chain + (1. - kW))
...
def pte1_thrust(aircraft, Pamb, Tamb, Mach, rating, nei):
    ...
    if (pw_shaft2 > 0.):
        ...
        if (propulsion.bli_effect > 0):
            (fn_fan2,
             q1,
             dVbli) = jet.fan_thrust_with_bli(e_nacelle,
                                              Pamb,
                                              Tamb,
                                              Mach,
                                              pw_shaft2)
        ...
        sec = (pw_shaft2 / (e_nacelle.motor_efficiency *
                            e_nacelle.controller_efficiency)) / fn_fan2
...
def rear_electric_nacelle_drag(aircraft, nacelle, Re, Mach):
    ...
    nac_cxf = 1.15 * ((0.455 / fac) * (numpy.log(10) /
                                       numpy.log(Re * nacelle.length))**2.58) * nac_nwa / wing.area
