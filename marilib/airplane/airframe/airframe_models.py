...
def htp_aero_data(aircraft):
    ...
    # Helmbold formula
    cza_htp = (numpy.pi * htp.aspect_ratio) / (1 +
                                               numpy.sqrt(1 + (htp.aspect_ratio / 2)**2)) * (htp.area / wing.area)
...
def vtp_aero_data(aircraft):
    ...
    cyb_vtp = vtp_lift_gradiant(aircraft)
...
def vtp_lift_gradiant(aircraft):
    ...
    # Helmbold formula
    cyb = (numpy.pi * vtp.aspect_ratio) / \
        (1 + numpy.sqrt(1 + (vtp.aspect_ratio / 2)**2)) * \
        (vtp.area / wing.area)
