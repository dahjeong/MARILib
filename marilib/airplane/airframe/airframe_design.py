...
def eval_cabin_mass(aircraft):
    ...
    cabin.m_furnishing = 0.063 * cabin.n_pax_ref**2 + \
        9.76 * cabin.n_pax_ref        # Furnishings

    cabin.m_op_item = 5.2 * \
        (cabin.n_pax_ref *
         design_driver.design_range *
         1e-6)     # Operator items

    # Rear cabin is heavier because of higher density
    cabin.cg_furnishing = cabin.fwd_limit + 0.55 * cabin.length
...
def eval_fuselage_design(aircraft):
    ...
    fuselage.net_wetted_area = 2.70 * fuselage.length * \
        numpy.sqrt(fuselage.width * fuselage.height)
...
def eval_fuselage_mass(aircraft):
    ...
    kfus = numpy.pi * fuselage.length * \
        numpy.sqrt(fuselage.width * fuselage.height)

    # Statistical regression versus fuselage built surface
    fuselage.mass = 5.47 * kfus**1.2
...
def eval_wing_design(aircraft):
    ...
    # Empirical law
    wing.sweep = 1.6 * max(0, (design_driver.cruise_mach - 0.5))
    ...
    wing.y_kink = 0.7 * fuselage.width + 1.4 * \
    ...
    if(15 < unit.deg_rad(wing.sweep)):  # With kink
        ...
        A = ((1 - 0.25 * wing.taper_ratio) * wing.y_kink + 0.25 * wing.taper_ratio *
             wing.y_root - wing.y_tip) / (0.75 * wing.y_kink + 0.25 * wing.y_root - wing.y_tip)
        B = (numpy.tan(wing.sweep) - tan_phi100) * ((wing.y_tip - wing.y_kink) *
                                                    (wing.y_kink - wing.y_root)) / (0.25 * wing.y_root + 0.75 * wing.y_kink - wing.y_tip)
        wing.c_root = (wing.area - B * (wing.y_tip - wing.y_root)) / (wing.y_root + wing.y_kink +
                                                                      A * (wing.y_tip - wing.y_root) + wing.taper_ratio * (wing.y_tip - wing.y_kink))
        wing.c_kink = A * wing.c_root + B
        wing.c_tip = wing.taper_ratio * wing.c_root

    else:		# Without kink
        wing.c_root = 2. * wing.area / (2. * wing.y_root * (1. - wing.taper_ratio) + (
            1. + wing.taper_ratio) * numpy.sqrt(wing.aspect_ratio * wing.area))
        wing.c_tip = wing.taper_ratio * wing.c_root
        wing.c_kink = ((wing.y_tip - wing.y_kink) * wing.c_root +
                       (wing.y_kink - wing.y_root) * wing.c_tip) / (wing.y_tip - wing.y_root)

    tan_phi0 = 0.25 * (wing.c_kink - wing.c_tip) / \
        (wing.y_tip - wing.y_kink) + numpy.tan(wing.sweep)

    wing.mac = 2. * (3. * wing.y_root * wing.c_root**2
                     + (wing.y_kink - wing.y_root) * (wing.c_root **
                                                      2 + wing.c_kink**2 + wing.c_root * wing.c_kink)
                     + (wing.y_tip - wing.y_kink) * (wing.c_kink **
                                                     2 + wing.c_tip**2 + wing.c_kink * wing.c_tip)
                     ) / (3 * wing.area)

    wing.y_mac = (3. * wing.c_root * wing.y_root**2
                  + (wing.y_kink - wing.y_root) * (wing.c_kink * (wing.y_root +
                                                                  wing.y_kink * 2.) + wing.c_root * (wing.y_kink + wing.y_root * 2.))
                  + (wing.y_tip - wing.y_kink) * (wing.c_tip * (wing.y_kink +
                                                                wing.y_tip * 2.) + wing.c_kink * (wing.y_tip + wing.y_kink * 2.))
                  ) / (3. * wing.area)
    ...
    if (c_g.cg_range_optimization == 0):
        wing.x_root = 0.33 * fuselage.length**1.1 - \
            (x_mac_local + 0.25 * wing.mac)
    ...
    wing.z_kink = wing.z_root + \
        (wing.y_kink - wing.y_root) * numpy.tan(wing.dihedral)
    wing.z_tip = wing.z_root + \
        (wing.y_tip - wing.y_root) * numpy.tan(wing.dihedral)
    ...
    cza_wo_htp = frame_aero.cza_wo_htp(
        mach,
        fuselage.width,
        wing.aspect_ratio,
        wing.span,
    ...
    wing.setting = (0.97 * mass * g) / (0.5 * gam * pamb *
                                        mach**2 * wing.area * cza_wo_htp) - unit.rad_deg(2.5)
...
def eval_wing_mass(aircraft):
    ...
    D = (0.6 * wing.t_o_c_r + 0.3 * wing.t_o_c_k + 0.1 * wing.t_o_c_t) * \
        (wing.area / wing.span)
    E = numpy.cos(wing.sweep)**2
    F = 1200. * (cz_max_ld - 1.8)**1.5

    # Shevell formula + high lift device regression
    wing.mass = A + (B * C) / (D * E) + F
...
def eval_vtp_design(aircraft):
    ...
    # Factor 2.01 accounts for carmans
    vtp.net_wetted_area = 2.01 * vtp.area
    ...
    vtp.mac = vtp.height * \
        (vtp.c_root**2 + vtp.c_tip**2 + vtp.c_root * vtp.c_tip) / \
        (3 * vtp.area)
    vtp.x_mac = vtp.x_root + \
        (vtp.x_tip - vtp.x_root) * vtp.height * \
        (2 * vtp.c_tip + vtp.c_root) / (6 * vtp.area)

    vtp.lever_arm = (vtp.x_mac + 0.25 * vtp.mac) - \
        (wing.x_mac + 0.25 * wing.mac)
...
def eval_vtp_statistical_sizing(aircraft):
    ...
    vtp.area = vtp.volume * \
        (1.e-3 * engine.reference_thrust * nacelle.y_ext) / vtp.lever_arm
...
def eval_htp_design(aircraft):
    ...
    # Empirical law
    wing_sweep = 1.6 * max(0, (design_driver.cruise_mach - 0.5))
    ...
    htp.mac = htp.span * \
        (htp.c_axe**2 + htp.c_tip**2 + htp.c_axe * htp.c_tip) / (3. * htp.area)
    htp.y_mac = htp.y_tip**2 * (2 * htp.c_tip + htp.c_axe) / (3 * htp.area)
    x_tip_local = 0.25 * (htp.c_axe - htp.c_tip) + \
        htp.y_tip * numpy.tan(htp.sweep)
    x_mac_local = htp.y_tip * x_tip_local * \
        (htp.c_tip * 2. + htp.c_axe) / (3. * htp.area)
    ...
    htp.lever_arm = (htp.x_mac + 0.25 * htp.mac) - \
        (wing.x_mac + 0.25 * wing.mac)
...
def eval_landing_gear_mass(aircraft):
    ...
    ldg.mass = 0.02 * weights.mtow**1.03 + \
        0.012 * weights.mlw    # Landing gears
...
def eval_wing_tank_data(aircraft):
    ...
    tanks.mfw_volume_limited = (
        tanks.central_volume + tanks.cantilever_volume) * tanks.fuel_density
    ...
    # Fuel max forward CG, central tank is forward only within backward swept
    # wing
    tanks.fuel_max_fwd_cg = tanks.fuel_central_cg
